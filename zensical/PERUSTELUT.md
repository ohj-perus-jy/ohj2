# Zensical-koeputki: perustelut

Koeputken työpäiväkirja: jokainen ratkaisu, sen vaihtoehdot ja se, millä
se todennettiin. Tila, käyttöohjeet ja puuttuvat kohdat ovat
...
vaan havaintoja. Sisältöä muunnetaan toistaiseksi kahdessatoista kohdassa
(sisällytykset, piilorivit, korostukset, välilehdet, alertit, avattavat osiot,
tehtäväkortit, vaatimuslohkot, kaaviot ja ikonit, ks. kohdat 1, 2, 4, 5, 6, 7,
8, 9, 15, 17, 23 ja 25); jokainen uusi muunnos kuuluu perustella samalla tavalla
kuin muutkin rivit.
...
Testit tarvitsevat neljää asiaa: `pytest`, `playwright`, sen chromium ja
selaimen systeemikirjastot. Kolme neljästä on `.venv`:ssä, mutta **selain ja
sen kirjastot ovat sen
...
Mitään ei jäljitellä (ks. README:n testitaulukko). Tulostussivu on koeputken
...
**Bonusmerkki ei tarvinnut ikonifonttia.** Lähteessä se on `<i class="bi
...
`<span class="task-bonus">`, jonka sisällä on merkki ja sana "Bonus"; merkin
piirtää Materialin `creation`-kuvake, ks. oma kohtansa alempana. Liuska jää nimen
...
Kohta 17 kutistui samalla: 150 ikonitagista 36 oli tehtävien bonustähtiä,
eivätkä ne enää tarvitse fonttia. Loput 114 odottivat tässä vaiheessa vielä
ratkaisuaan; niistä 30 hoitui seuraavassa kohdassa ja 84 sitä seuraavassa.

### Bonusmerkki Materialin kuvakkeella (uusi `convert_bonus_marks` + 1 sääntö `assets/css/tasks.css`:ään)

Bonusmerkki on lähteessä `<i class="bi bi-stars">` kolmessa paikassa: tehtävän
nimen sisällä (36), avattavien lohkojen `<summary>`-riveillä (21) ja
harjoitustyön vaatimuslistoissa (9). Edellinen kohta hoiti näistä
ensimmäisen liuskan `::before`-tähdellä (`\2605`), loput 30 jäivät tyhjäksi
tilaksi sivulle.

**Merkiksi valittiin Materialin `creation`.** Vaihtoehdot katsottiin rinnakkain
aidoilla glyfeillä kolmessa oikeassa yhteydessä (kappale, vaatimuslistan rivi,
kortin liuska) molemmilla teemoilla. Ratkaisevaa oli kaksi asiaa: `creation` on
kolme neljäsakaraista kimallusta samassa sommittelussa kuin kirjan `bi-stars`,
ja se kestää liuskan koon — liuskan teksti on 0,62 × otsikko, jolloin merkki
piirtyy noin kymmenen pikselin kokoisena, ja siinä koossa `lucide-sparkles`
menee puuroksi ja `octicons-star-16` lukee arvosteluna eikä bonuksena.
Viisisakarainen `\2605` erottui kirjasta selvimmin. Kuvake tulee Zensicalin
omasta ikonihakemistosta, joten mitään ei asenneta eikä ladata.

**Merkki kirjoitetaan inline-SVG:nä eikä lyhytkoodina**, vaikka teema osaa
lyhytkoodin `:material-creation:` ilman riviäkään `mkdocs.yml`:ään
(`pymdownx.emoji` on Zensicalin oletuslistalla). Syy on `<summary>`: Python-Markdown ei jäsennä sen sisältöä, koska
`markdown="1"` on `<details>`-tagissa ja `md_in_html` käsittelee sillä vain
lohkotason lapset. Mitattuna koesivulla lyhytkoodi kääntyi listassa ja
kappaleessa mutta jäi `<summary>`-rivillä raakana näkyviin — ja siellä on 21
merkkiä kolmestakymmenestä. Sama merkintä joka paikkaan on siis ainoa, joka
kääntyy joka paikassa. Kääre on teeman oma `.twemoji`, eli tasan se minkä
lyhytkoodi tuottaisi, joten koon, kohdistuksen ja `fill: currentcolor`in hoitaa
teeman CSS; omaa sääntöä tarvitaan vain väriin.

**Ruudunlukija saa merkistä enemmän kuin kirjassa.** Kirjassa merkki on
ikonifontti ilman nimeä, eli se ei välity mitenkään. Tässä merkki nimetään
silloin kun rivillä ei ole samaa tietoa tekstinä: `<summary>`-rivit alkavat
sanalla "Bonus:" tai "Valinnaista lisätietoa:", jolloin nimi vain toistaisi
otsikon ja merkki on `aria-hidden`, mutta harjoitustyön vaatimuslistassa merkki
on rivin ainoa ero pakolliseen vaatimukseen, jolloin se saa
`role="img" aria-label="Bonus"`. Liuskassa merkki on aina koriste, koska sana
"Bonus" lukee siinä vieressä.

Todennettu neljällä tavalla:

- **Kaikki merkit muuntuivat:** ajossa 30 merkkiä korttien ulkopuolella ja 36
  liuskaa, yhteensä tasan ne 66 `bi-stars`-tagia jotka lähdepuussa on.
  `docs/`:ssä ei ole enää yhtään `bi-stars`-luokkaa, ja käännetyssä sivustossa
  on 66 `.jyu-bonus`-merkkiä.
- **Käännöksen varoitukset eivät muuttuneet:** 16 ennen ja jälkeen.
- **Testit:** 158 läpi, joista neljä uutta `test_convert.py`:ssä (liuskan
  merkki, nimeäminen kummassakin suunnassa, koodiaidan ohitus).
- **Selaimessa mitattuna** kaikissa kolmessa yhteydessä ja molemmissa
  teemoissa: liuskassa merkki istuu sanan "Bonus" vasemmalla puolella
  kimalluksineen erottuvana, vaatimuslistassa se on rivin alussa
  peruslinjalla, ja `<summary>`-rivillä otsikon edessä.

**Sivutuote: `markdown_extensions`ia ei saa kirjoittaa `mkdocs.yml`:ään
kevyesti.** Lyhytkoodivaihtoehtoa kokeiltaessa selvisi, että oma
`markdown_extensions`-lohko **korvaa** Zensicalin koko oletuslistan sen sijaan
että täydentäisi sitä (`config.py`: lista alkaa aina `["toc", "tables"]`:sta).
Mitattuna yhden rivin (`- attr_list`) lisääminen pudotti yhden sivun
välilehtijoukot viidestä kahteen, koska `pymdownx.tabbed` katosi samalla.
Jos listaan joskus koskee, sinne on kirjoitettava kaikki 20 oletuslaajennusta
`pymdownx.emoji`in generaattorifunktioita myöten.

Kohta 17:stä on nyt tehty 66 ikonia 136:sta. Loput 70 odottavat yhä omaa
ratkaisuaan: 49 `bi-chevron-right` valikkopoluissa (`tyokalut.md`), 5
`bi-play-fill` ja 16 muuta yksittäistä.
...
### Loput ikonit: nuoli merkiksi, kuvakkeet teeman glyfeiksi (uusi `assets/css/icons.css` + uusi `icons/` + ~240 riviä `convert.py`:hyn + 1 rivi `mkdocs.yml`:ään)

Lähdepuussa on **150 ikonitagia**, ei 136 kuten listalla aiemmin luki. Ero on
laskutavassa: `<i>`-tageista 19 on rivitetty kesken tagin, ja rivi kerrallaan
etsivä laskenta ohittaa ne. Samasta syystä listalta puuttui kolmas
ikonikirjasto kokonaan — Font Awesomea on kolmessa kohdassa (`fa-play`,
`fa-eye`, `fa-history`), ja niistä yksi oli piilossa juuri rivinvaihdon takana.

Bonusmerkki (`bi-stars`, 66) hoitui edellisessä kohdassa. Loput **84** ovat
tämä kohta, ja niiden jako on koko ratkaisun ydin: 58 niistä ei ole kuvakkeita
lainkaan.

**Valikkopolun nuoli on välimerkki.** `bi-chevron-right` (56) ja
`bi-arrow-right` (2) tekevät lähteessä yhtä ja samaa työtä: ne erottavat
valikon kohdat toisistaan (*File › New › Project*). Se on välimerkin tehtävä,
ja siihen riittää merkki — ei fonttia, ei SVG:tä, ei tavuakaan verkosta.

**Merkiksi valittiin `›` (U+203A), ja ratkaisu on kirjasimessa.** Zensical
hakee leipätekstin Source Serif 4:n Google Fontsista, ja mitattuna
(`fonts.googleapis.com/css2?family=Source+Serif+4`) sen `latin`-osajoukon
`unicode-range` kattaa alueet U+0000–00FF ja U+2000–206F. `›` osuu
jälkimmäiseen ja `»` (U+00BB) edelliseen, eli kirjan oma kirjasin piirtää
molemmat. Nuoli `→` (U+2192) ei osu kumpaankaan — osajoukon nuolista mukana
ovat vain `↑` ja `↓` — eikä kolmio `▸` (U+25B8), joten kumpikin vaihtaisi
kirjasinta kesken virkkeen ja toisi mukanaan varakirjasimen painon ja
x-korkeuden. Merkki jää lisäksi *tekstiksi*: se menee hakuindeksiin ja
ruudunlukija saa siitä välimerkin, kun ikonifontin glyfi oli kummallekin
tyhjää.

**Väri vaimennetaan, koska polun kohdat ovat lihavoituja.** Erotin on
`<span class="jyu-path">`, ja kääre on olemassa vain väriä varten: koon,
painon, kohdistuksen ja rivityksen hoitaa leipätekstin oma kirjasin.
Lähteessä polun kohdat ovat usein `**File** › **Settings**`, jolloin täydellä
tekstivärillä erotin painaisi rivillä yhtä paljon kuin ne kohdat, joita sen on
tarkoitus erottaa. Väri on teeman oma `--md-default-fg-color--light`, ja se
riittää molemmissa teemoissa: selaimessa mitattuna erotin on vaaleassa
`rgb(102, 102, 102)` valkoista vasten (5,7:1) ja tummassa `rgb(144, 146, 150)`
...
oikea glyfi ei ole "lähin Materialin vastine" vaan *se sama glyfi, jonka
sivusto itse piirtää siihen nappiin*:

| Ohjeen kohta | Glyfi | Mistä sivusto piirtää saman |
| ------------ | ----- | --------------------------- |
| Sisällysluettelo, hampurilaisvalikko | `material/menu` | `overrides/partials/header.html` |
| Haku | `material/magnify` | `overrides/partials/header.html` |
| Tulosta | `lucide/printer` | `overrides/partials/header.html` |
| Teema | `material/weather-night` | `mkdocs.yml`: `palette.toggle.icon` |
| Aja koodi | `material/play` | `assets/css/playground.css`: `--jyu-run-icon` |
| Näytä koko koodi | `material/eye-outline` | `assets/css/hidelines.css`: `--jyu-eye-icon` |

Loput ovat IntelliJ:n ja SceneBuilderin painikkeita, joille valittiin Materialin
lähin vastine: `bug`, `folder-outline`, `console-line`, `cog`,
`lightbulb-on-outline`, `information-outline`. Ääriviivaversio siellä missä
valinta on — täytetty `lightbulb-on` ja `folder` ovat leipätekstissä
ympäristöään selvästi painavampia. Lähteen kaksi eri kansiokuvaketta
(`bi-folder`, `bi-folder2`) tarkoittavat samaa IntelliJ:n nappia, joten ne
saavat saman glyfin.

**Inline-SVG eikä lyhytkoodi, samasta syystä kuin bonusmerkissä.** Kaksi
merkeistä on `<details>`-lohkon `<summary>`-rivillä, jota Python-Markdown ei
jäsennä, joten `:material-information-outline:` jäisi siellä raakana näkyviin.
Kääre on teeman oma `.twemoji`, joten koon (`--md-icon-size`), kohdistuksen
(`vertical-align: text-top`) ja värin (`fill: currentcolor`) hoitaa teeman CSS
— eikä Luciden viivapiirroskaan tarvitse mitään lisää, koska teemalla on sille
valmis sääntö (`.twemoji svg.lucide { fill: #0000; stroke: currentcolor }`).
Kuvake on aina koriste (`aria-hidden`), koska nappi on jokaisessa kohdassa
nimetty samalla rivillä sanoina.

**Glyfit ovat kopioina `icons/`-hakemistossa.** Suoraan teemalta lukeminen
olisi ollut houkuttelevaa, mutta `run.sh` ajaa `python3 convert.py` **systeemin
Pythonilla** eikä `.venv`:stä: `import zensical` kaatuisi siihen, ja `.venv`:n
polun arvaaminen sitoisi skriptin virtuaaliympäristön sisäiseen rakenteeseen.
Hakemistossa on 13 tiedostoa sellaisenaan Zensical 0.0.60:n
`templates/.icons/`:sta, ja testi vertaa jokaista niistä asennettuun teemaan,
joten ero näkyy heti kun teema vaihtaa glyfiä.

**Etusivun navigointiohje poistuu kokonaan** (uusi `DROP_SECTIONS`,
`drop_sections`). `index.md`:n osio "Navigointi tässä materiaalissa" kertoo,
että sivuja selataan "nuolikuvakkeista sivun vasemmassa ja oikeassa laidassa" —
Zensicalissa laitanuolia ei ole lainkaan, vaan `navigation.footer` laittaa
edellisen ja seuraavan linkit aina alalaitaan, myös työpöydällä. Kuvakkeen
vaihtaminen olisi korjannut merkin muttei väitettä. Osion kaksi muuta neuvoa
(sisällysluettelo ja haku) ovat jo osan 1 etusivun listassa, joten osio jää
pois kokonaan ja neljä ikonitagia sen mukana. Lähde ei muutu (`../src` on
kirjan oma), joten poisto tehdään muunnoksessa: avaimena sivun polku, arvona
otsikko, ja mukaan lähtee otsikko alaotsikoineen seuraavaan samantasoiseen
...
- **Jokainen tagi muuntui:** ajossa 58 nuolta ja 22 kuvaketta, tasan se mitä
  lähdepuussa on poiston jälkeen. `docs/`:ssä ei ole enää yhtään
  `<i class="bi ...">`- tai `<i class="fa ...">`-tagia, eikä ajo ilmoittanut
  yhtään tuntematonta ikonia.
- **Käännöksen varoitukset eivät muuttuneet:** 16 ennen ja jälkeen.
- **Sama glyfi kuin napissa, mitattuna selaimesta:** `osa1/index`-sivun
  tulostuskuvakkeen `innerHTML` on merkilleen sama kuin yläpalkin
  tulostusnapin, ja ajonapin polku (`M8 5.14v14l11-7z`) on sekä sivun tekstissä
  että `playground.css`:n muuttujassa.
- **Erotin tulee leipätekstin kirjasimesta:** `.jyu-path`:n laskettu
  `font-family` on `"Source Serif 4"` ja `font-size` 15 px, sama kuin
  ympäröivällä kappaleella.
- **Testit:** 174 läpi, joista kuusitoista uutta `test_convert.py`:ssä
  (nuoli merkkinä, rivitetty ja lainauslohkossa rivitetty tagi, kuvake teeman
  kääreessä, koodiaitojen ohitus ja aidan raja, tuntematon ikoni ja puuttuva
  glyfi, ajojärjestys bonusmerkkien jälkeen, toistettavuus, glyfit samat kuin
  teemalla ja samat kuin sivuston napeissa, osion poisto alaotsikoineen,
  poiston sivukohtaisuus, ristikko koodiaidassa, varoitus kadonneesta osiosta).

Mitä jäi auki: **teemanapilla on kaksi tilaa mutta tekstillä yksi glyfi.**
Yläpalkin nappi on vaaleassa teemassa kuu ja tummassa aurinko; ohjeessa on kuu.
Sama koskee virkettä "Vaihda ulkoasua (vaalea, tumma, automaattinen)" —
Zensicalin nappi on kaksiasentoinen ja automaattinen tulee
käyttöjärjestelmästä. Ja **`fa-history` osoittaa nappiin, jota ei ole**: se on
muokattavan koodilohkon "Peruuta muutokset", ja ACE-editori on yhä tekemättä
(kohta 20). Kuvake piirretään silti, koska tyhjä väli ei kerro lukijalle
...
## Ratkaistu: muunnosaskel pois käsistä (vaihtoehto A)

Tämä luku oli auki siihen asti, kunnes ratkaiseva luku mitattiin. Mittaus on
alla, ja se valitsi vaihtoehdon A: `convert.py` jää, ja sen ympärillä on nyt
vahti (`convert.py --watch`, run.sh käynnistää sen palvelimen rinnalle).
Lähtökohta oli tämä:
...
**Tämä valittiin**, ks. mittaus luvun lopussa; toteutus on `convert.py`:n
`watch` ja sen kolme apuria.
...
**Ratkaiseva luku oli tämä: kauanko tallennuksesta kuluu selaimen
päivittymiseen**, kun `zensical serve` huomaa muutoksen. Jos se on sekunnin
luokkaa, A riittää. Jos se on kymmeniä sekunteja, C on perusteltu, koska
silloin käännettävää on vain muuttunut sivu.

Mitattuna (devcontainer, ext4, Zensical 0.0.60; sivu `osa1/01-hei-java.md`,
kolme toistoa):

| Mitä                                                  | Aika       |
| ----------------------------------------------------- | ---------- |
| muutos `docs/`:iin -> palvelin tarjoilee sen           | 0,2–0,6 s  |
| `convert.py` koko kirjalle                             | 0,33 s     |
| koko kierros `../src` -> vahti -> selain               | 1,1–1,4 s  |
| `zensical build`, välimuisti kylmänä                   | 4,4 s      |
| `zensical build`, mikään ei muuttunut                  | 1,4 s      |

`serve` kääntää siis vain muuttuneen sivun, ei koko kirjaa. Aiemmat
vertailuluvut (kylmänä 24 s, muuttumattomana 14 s) olivat noin viisinkertaisia
näihin nähden; ne on korvattu yllä olevilla.

**Vahti vaati kaksi korjausta itse muunnokseen.** Kumpikaan ei liity
nopeuteen, ja molemmat löytyivät vasta kun muunnos alkoi ajautua itsestään:

*Muunnos kirjoitti joka ajolla koko puun.* `sync_docs` kopioi kaikki 600
tiedostoa ja main kirjoitti sivut vielä toiseen kertaan muunnettuina, eli
yksi tallennus tuotti 600 kirjoitusta. Käsin ajettuna se ei haitannut, mutta
`zensical serve` ilmoittaa jokaisesta muuttuneesta tiedostosta selaimelle
erikseen: mitattuna 500-1300 WebSocket-viestiä tallennusta kohti. Selain
latasi sivun kesken käännöksen ja sai vanhan sisällön, ja oikea ilmoitus
hukkui joukkoon — muutos näkyi vasta seuraavasta tallennuksesta. Nyt
kirjoitetaan vain muuttunut (`copy_if_changed`, `write_if_changed`, ja main
lukee sivut lähdepuusta eikä docs/:n raakakopiosta): yhden sivun muutos on
yksi kirjoitus, muuttumaton ajo ei kirjoita mitään, ja ajo nopeutui samalla
1,2 sekunnista 0,33 sekuntiin.

*Kaksi yhtäaikaista ajoa tuhosi versionhallinnassa olevia tiedostoja.* Kun
kaksi `run.sh`:ta oli auki, toinen ajo luki sivun, jonka ensimmäinen oli jo
muuntanut. Muunnetussa sivussa ei ole `bob`-aitaa, joten kaavio ei päätynyt
käytettyjen joukkoon, ja `prune_diagrams` poisti sen välimuistista: kymmenen
`cache/svgbob/`-tiedostoa katosi. Korjauksia on kaksi. Muunnokset ajetaan
yksi kerrallaan (`only_one_run`, tiedostolukko), ja siivous tehdään vain jos
kaikki kaaviot saatiin piirrettyä (`prune_diagrams`: `complete`) — vajaa
joukko ei kerro, mikä on käyttämätöntä, ja ilman piirtäjää poistuisivat juuri
ne tiedostot, joita ilman piirtäjää eniten tarvitaan.

**Tulos: A riittää, eikä C:lle jää nopeusperustetta.** C:n ainoaksi
perusteluksi jää `docs_dir: src` (alla), joka on eri kysymys kuin nopeus.

Mittaus vaatii, että rinnalla ei ole toista `run.sh`:tä: kaksi vahtia ja kaksi
palvelinta samassa `docs/`:ssa venytti kierroksen 5–8 sekuntiin ja jätti yhden
muutoksen kokonaan näkymättä.
...
kerätään kaikilta sivuilta ennen renderöintiä — eli esiajo tarvittaisiin
sittenkin.
...
tahansa. Polku `/workspaces/...` ei kerro kumpi tilanne on käsillä:
`findmnt -no FSTYPE,SOURCE --target /workspaces` näyttää sen, ja tässä
devcontainerissa vastaus on ext4 docker-volumessa.

Oma vahti ei silti nojaa inotifyyn vaan kysyy muokkausajat 0,3 s välein
(`convert.py`: `snapshot`). Se on halpa — 600 tiedostoa, 6 ms — ja toimii myös
9p:n takana. `serve`in omaa vahtia se ei korvaa: väärässä ympäristössä
muunnos ajettaisiin, mutta palvelin ei huomaisi `docs/`:n muuttuneen.
