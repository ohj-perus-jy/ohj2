...
void main() {
    ...
    // TODO: tarkista, että tämä syntaksi esitellään osassa 1
...
public class Kerailykortti implements Comparable<Kerailykortti> {
// HIGHLIGHT_GREEN_END
    private String nimi;
    private int tunnistenumero;
    public Kerailykortti(String nimi, int tunnistenumero) {
...
Rajapinnan käyttö on suoraviivaista käyttäen sen oletustoteutuksia `comparing` sekä `thenComparing`, jotka mahdollistavat vertailun määrittelyn helposti lambda-lausekkeilla. Alla esimerkki, jossa määritellään korttien järjestys ensin sarjan nimen ja sitten aakkosjärjestyksen mukaan käyttäen `Comparator`-rajapintaa.
...
void main() {
    ...
    //HIGHLIGHT_GREEN_BEGIN
    Collections.sort(kortit, Comparator.comparing((Kerailykortti k) -> k.getSarja())
        .thenComparing(k -> k.getNimi()));
    //HIGHLIGHT_GREEN_END
